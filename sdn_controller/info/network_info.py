...
def get_shortest_path(kme_src: UUID, kme_dst: UUID) -> list[UUID]:
    ...
    global all_paths
    try:
        # l: list[UUID] = nx.shortest_path(G, kme_src, kme_dst)
...
def get_path(kme_src: UUID, kme_dst: UUID, req_rate: float) -> list[UUID]:
    ...
    for i in range(0, len(path) - 1):
        ...
        if not __satisfiable_request(req_rate=req_rate, rate=rate, used_rate=used_rate):
            raise HTTPException(
                status_code=500,
